function EvernotePageInfo() {
  ...
  Array.prototype.indexOf = function (searchElement /*, fromIndex */ ) {
      ...
      //console.log("my indexOf from PageInfo");
      var t = Object(this);
      var len = t.length >>> 0;
      if (len === 0) {
          return -1;
  ...
  var specialCases = {
    ...
    // special Evernote Food recipe cases
    "blog.evernote.com"    : [".post"],
    "recipe.rakuten.co.jp" : ["div#mainContents"],
    "erecipe.woman.excite.co.jp" : ["section#detail"],
    "douguo.com" : [".recinfo"],
    "icook.tw" : [".recipe-detail"]
  }
  //    "kirei.biglobe.ne.jp": [".permalink", "div#main"],
  ...
  // Hack to make ' selection clipping work. Keeps track of the last text that was selected so that we'll still
...
//  }
  ...
  function clearlyCallback(data, callback) {
    ...
    console.log("prototype used at="+(new Date()).valueOf());
    var specialCaseEntry = specialCasesInsertBox[host];
    if (!specialCaseEntry) {
      console.log("not in base special case map, check food specific one");
      specialCaseEntry = evernoteFoodSpecialCases[host];
    }
    if (specialCaseEntry) {
      console.log("host is in the special list");
      for (var flavor = 0; flavor < specialCaseEntry.length; flavor++) {
        console.log(specialCaseEntry[flavor].container);
        var container = document.querySelector(specialCaseEntry[flavor].container);
        if (container) {
          try {
            console.log("special");
            var include = container.querySelectorAll(specialCaseEntry[flavor].include);
            if (include && include.length > 0) {
              ...
              var divStyle = specialCaseEntry[flavor].divStyle
              if(divStyle){
                article.style.cssText = divStyle;
              }
              ...
              insertBefore.parentNode.insertBefore(article, insertBefore);
              break;
            }
          } catch (e) {
            console.log(e);
            console.log("Error handling special case");
            article = null;
          }
        }
      }
    ...
    // Or see if it's a special case image page.
      ...
      console.log("prototype used at="+(new Date()).valueOf());
      console.log("useFoundImage.indexOf(host) != -1");
      article = findImage();
      if (article)
        articleBoundingClientRect = article.getBoundingClientRect();
    }
    ...
    if (!article) {
      console.log("3rd !article");
      console.log("elements length="+data._elements.length);
  ...
  Browser.addMessageHandlers({
    ...
    content_ready_5_8: readyRequestHandler
  });
...
//        var trimmed = node.textContent.trim().replace(/\s+/g, " ");
  ...
  */
  ...
  // Public API:
  me.getDefaultArticle = getDefaultArticle;
  me.getSelection = getSelection;
  me.getSelectionFrame = getSelectionFrame;
  me.getFavIconUrl = getFavIconUrl;
