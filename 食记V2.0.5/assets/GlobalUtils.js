...
function GlobalUtils() {
  ...
  me.localize = function(element) {
    ...
    for (var i = 0; i < element.children.length; i++) {
      me.localize(element.children[i]);
    }
  };

  me.getQueryParams = function(url) {
    var data = me.componentizeUrl(url);
  ...
  me.createUrlClipContent = function(title, url, favIcoUrl) {
    var titleAttr = (title) ? me.escapeXML(title) : "";
  ...
  me.getNoteURI = function(client, baseUrl, message, action, userId) {
    var uri;
    if (client == "WEB") {
  ...
  me.buildGoogleRegEx = function() {
    ...
    return new RegExp("^https?://([^./]+\\.)?google(" + googleCountryDomains.join("|") + ")/", "i");
  };

  me.removePunctuation = function(text) {
    var dashes = ["-", "_", "\u2013", "\u2014", "\u00B7"];
    var brackets = ["\\(", "\\)", "\\[", "\\]", "\\{", "\\}", "\u300A", "\u300B",
      "\uFF08", "\uFF09", "\u3010", "\u3011", "\u300C", "\u300D", "\u00BB"];
    var punc = ["\\.", "!", "\:", "\;", "\"", "\'", ",", "\\?", "\u3002",
